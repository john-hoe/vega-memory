...
AI 코딩 도구에 **영구적인 크로스 세션 메모리**를 제공하는 로컬 우선 메모리 서버입니다. Cursor, Claude Code, Codex, OpenClaw 또는 모든 MCP 호환 클라이언트를 연결하세요 — 모든 도구가 동일한 지식 베이스를 공유하므로, 한 세션에서 학습한 내용이 다음 세션에서 사라지지 않습니다.
