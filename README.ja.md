...
ローカルファーストのメモリーサーバーで、AIコーディングツールに**永続的なクロスセッションメモリー**を提供します。Cursor、Claude Code、Codex、OpenClaw、またはMCP対応クライアントを接続すれば、すべて同じナレッジベースを共有し、あるセッションで学んだことが次のセッションで失われることはありません。
